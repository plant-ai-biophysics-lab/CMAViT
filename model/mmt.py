...
class TextEmbed(nn.Module):
    def __init__(self, config):
        ...
        self.model_embedding = DistilBertModel.from_pretrained("distilbert-base-uncased").to(device)

    def forward(self, texts):
        # Encode batch of texts
        encoded_input = self.tokenizer.batch_encode_plus(texts, return_tensors='pt', padding=True, truncation=True)
        input_ids = encoded_input['input_ids'].to(device)
        
        # Get model's embeddings
        outputs = self.model_embedding(input_ids = input_ids)
        
        # Extract the embeddings of the last hidden state
...
class MMTR(nn.Module):
    def __init__(self, config):
        super(MMTR, self).__init__()
        self.config = config

        if self.config.tokenizer == 'CAC': 
            self.transformer = nn.ModuleDict(dict(
                img_embed = ImgEmbed(self.config), 
                text_embed = TextEmbed(self.config),
                h_cross = nn.ModuleList([CrossAttnBlock(config) for _ in range(config.num_layers)]),
    ...
    def forward(self, img, text):
        ...
        if self.config.tokenizer == 'EC':
            img = self.transformer.img_embed(img) 
        ...
        elif self.config.tokenizer == 'CAC':

            img = self.transformer.img_embed(img)
            text = self.transformer.text_embed(text)


            for block in self.transformer.h_cross:
                x_img = block(img, text)

            for block in self.transformer.h_cross:
                x_text = block(text, img)
        ...
        x = x.mean(dim = 1)
        output = self.transformer.head(x)
